...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    ...
    if ([self.delegate respondsToSelector:@selector(horizontalPickerViewWillBeginDragging:)]) {
        [self.delegate horizontalPickerViewWillBeginDragging:self];
    }
}
...
- (void)addScrollView {
	if (_scrollView == nil) {
		...
		_scrollView.decelerationRate = 3.0; //UIScrollViewDecelerationRateNormal;
...
- (void)scrollViewTapped:(UITapGestureRecognizer *)recognizer {
	if (recognizer.state == UIGestureRecognizerStateRecognized && !_scrollView.decelerating) {
...
- (void)scrollViewLongPressed:(UILongPressGestureRecognizer *)recognizer {
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        if ([self.delegate respondsToSelector:@selector(horizontalPickerViewLongPressed:)]) {
            [self.delegate horizontalPickerViewLongPressed:self];
        }
    }
}
